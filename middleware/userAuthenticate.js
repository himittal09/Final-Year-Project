...
const userAuthenticate = (request, response, next) => {
    ...
    //if the authenticated user is admin, finish route with return keyword, and send empty response with Unauthorised status code
    if (request.session.userLevel == 0) return response.status(401).send();
